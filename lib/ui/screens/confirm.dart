...
class _ConfirmScreenState extends State<ConfirmScreen> {
  ...
  Widget _buildCouponTextField() {
    return Padding(
      ...
      child: TextField(
        decoration: InputDecoration(labelText: 'كوبون الخصم'),
  ...
  List<Widget> _buildAddresses(context, model) {
    ...
    widgets.add(_buildHeader('عنوان التوصيل'));
    widgets.add(_buildContent(model.address['full_address']));
    widgets.add(Divider());
    widgets.add(_buildHeader('دفع'));
    widgets.add(_buildContent(formData['gateway']));
    widgets.add(Divider());

    widgets.add(_buildHeader('الشحن'));
    widgets
                                  ...
                                  children: <Widget>[
                                    ...
                                    Padding(
                                        padding: EdgeInsets.symmetric(
                                            horizontal: 25),
                                        child: model.coupon.type == 'percent'
                                            ? Text(
